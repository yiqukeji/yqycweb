...
public class MenuService implements MenuManager{
	...
	public List<Menu> listSubMenuByParentId(String parentId) throws Exception {
	 ...
	 @SuppressWarnings("unchecked")
		public List<Menu> listByStateMenuF(String State) throws Exception {
			return (List<Menu>) dao.findForList("MenuMapper.listByStateMenuF", State);
		}
	 @SuppressWarnings("unchecked")
		public List<Menu> listByF_StateMenuF(String F_STATE) throws Exception {
			return (List<Menu>) dao.findForList("MenuMapper.listByF_StateMenu", F_STATE);
		}
	/**
	 * 通过菜单ID获取数据
	 * @param pd
	 ...
	 * 
	 */
	public List<Menu> listByStateMenu(String state) throws Exception {
		List<Menu> menuList = this.listByStateMenuF(state);
		for(Menu menu : menuList){
			menu.setMENU_URL("menu/toEdit.do?MENU_ID="+menu.getMENU_ID());
			menu.setSubMenu(this.listAllMenu(menu.getMENU_ID()));
			menu.setTarget("treeFrame");
	...
	@Override
	public List<Menu> listByF_StateMenu(String F_STATE) throws Exception {
		List<Menu> menuList = this.listByF_StateMenuF(F_STATE);
		for(Menu menu : menuList){
			menu.setMENU_URL("menu/toEdit.do?MENU_ID="+menu.getMENU_ID());
			menu.setSubMenu(this.listAllMenu(menu.getMENU_ID()));
			menu.setTarget("treeFrame");
