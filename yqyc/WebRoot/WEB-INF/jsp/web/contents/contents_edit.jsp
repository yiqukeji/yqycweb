					...
					<form action="contents/${msg }.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr >
								...
								<td>
								<select class="chosen-select form-control" name="MENU" id="MEENU" data-placeholder="请选择栏目" style="vertical-align:top;" style="width:94%;" >
								<option value=""></option>
								<c:forEach items="${menu}" var="var">
									<option value="${var.NAME}" <c:if test="${var.NAME == pd.MENU }">selected</c:if>>${var.NAME }</option>
								</c:forEach>
								</select>
								</td>
							</tr>
