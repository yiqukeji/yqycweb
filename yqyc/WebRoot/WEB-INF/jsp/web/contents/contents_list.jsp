			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="contents/list.do?PUB_TYPE=${pd.PUB_TYPE}" method="post" name="Form" id="Form">
						<input name="PUB_TYPE" hidden="hidden" id="PUB_TYPE" value="${pd.PUB_TYPE}"/>
						<input name="C_STATE" hidden="hidden" id="C_STATE" value="${pd.C_STATE}"/>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr >
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.CONTENTS_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class='center' style="text-align: center;">
											<p title="${var.TITLE}" style="display: block;margin:auto;width: 427px;overflow: hidden;white-space: nowrap;text-overflow: ellipsis;">
											${var.TITLE}
											</p>
											</td>
											<td class='center'>${var.PUBLISHER}</td>
											<td class='center'>${var.CRE_TIME}</td>
											<td class='center'>${var.PUB_TIME}</td>
											<c:if test="${0 == var.PUB_STATE}">
											<td class='center'>
											审核
											</td>
											</c:if>
											<c:if test="${1 == var.PUB_STATE}">
											<td class='center'>
											发布
											</td>
											</c:if>
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.CONTENTS_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.CONTENTS_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		function add(){
			...
			var PUB_TYPE =$("#PUB_TYPE").val();
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>contents/goAdd.do?PUB_TYPE='+PUB_TYPE+'&C_STATE='+C_STATE;
			 diag.Width = 860;
			 diag.Height = 680;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>contents/delete.do?CONTENTS_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id,PUB_TYPE){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>contents/goEdit.do?CONTENTS_ID='+Id+'&PUB_TYPE='+PUB_TYPE;
			 diag.Width = 860;
			 diag.Height = 680;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>contents/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>contents/excel.do';
		}
	</script>
